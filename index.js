...
function updateSlider(sliderElement, toggleState){
    ...
    if ( localStorage.getItem(sliderElement.id) === 'true') {
        sliderElement.style.backgroundColor = 'rgba(189, 188, 188, 0.1)';
        ...
        sliderElement.style.backgroundColor = 'rgba(220, 220, 220, 0.5)';
...
function createBleAgent() {
    ...
    async function connectBLE() {
        ...
        try {
            ...
            isConnectedBLE = true;
            buttonBLE.innerHTML = '❌';
            displayBleStatus(`Connected to <br> ${device.name}`, 'rgba(77, 174, 80, 0.2)'); //green

        } catch (error) {
            if (error.name === 'NotFoundError') {
                displayBleStatus('No Device Selected', 'rgba(235, 91, 91, 0.2)');
            } else if (error.name === 'SecurityError') {
                displayBleStatus('Security error', 'rgba(235, 91, 91, 0.2)');
            } else {
                console.log( error);
                displayBleStatus('Connection failed', 'rgba(235, 91, 91, 0.2)');
                connectBLE();
            }
        }
    }
    ...
    async function disconnectBLE() {
        ...
        try {
            ...
            displayBleStatus("Error", 'rgba(235, 91, 91, 0.2)');
    ...
    function batteryWatchdogReset() {
        displayBleStatus('Connected', 'rgba(77, 174, 80, 0.2)'); //green
    ...
    function getGamepadAxes() {
        ...
        if (gamepad) {
            ...
            for (let i = 0; i < 4; i++) {
                ...
                barElements[i].style.background = `linear-gradient(to right, rgba(189, 188, 188, 0.3) ${percentage}%, var(--alf-green) 0%)`;
    ...
    function getButtonBytes() {
        ...
        if (gamepad) {
            ...
            for (let i = 0; i < buttonCount; i++) {
                ...
                if (buttonElements[i]) {
                    const newColor = button && button.pressed ? 'rgba(189, 188, 188, 0.3)' : 'rgba(189, 188, 188, 0.1)';
